...
export async function saveProduct(input: ProductEditorInput, productId?: string) {
  ...
  const saved = await prisma.$transaction(async (tx) => {
    const data = {
      ...
      printfulTemplateId: input.printfulTemplateId || null,
      printfulEnabled: input.printfulEnabled || input.fulfillmentType === "printful",
      printfulNeedsVariantMapping: input.printfulNeedsVariantMapping,
    ...
    if (input.variants.length) {
      await tx.productVariant.createMany({
        data: input.variants.map((variant, index) => ({
          ...
          printfulRetailPriceCents: variant.printfulRetailPriceCents ?? null,
          printfulCurrency: variant.printfulCurrency || null,
          printfulEnabled: variant.printfulEnabled || Boolean(variant.printfulVariantId || variant.printfulSyncVariantId),
...
function mapProductForEditor(product: ProductWithEditorRelations) {
  return {
    ...
    printfulNeedsVariantMapping: product.printfulNeedsVariantMapping,
    ...
    variants: product.variants.map((variant) => ({
      ...
      printfulSku: variant.printfulSku ?? "",
      printfulRetailPriceCents: variant.printfulRetailPriceCents ?? undefined,
      printfulCurrency: variant.printfulCurrency ?? "",
      printfulEnabled: variant.printfulEnabled,
      active: variant.active,
