...
import { z } from "zod";
import { logRoleRevoked } from "@/lib/audit/audit-log";
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid role revoke request.", issues: parsed.error.issues }, { status: 400 });
  }

  if (!hasDatabaseUrl()) {
    return NextResponse.json({ error: "DATABASE_URL is required to revoke roles." }, { status: 503 });
  }
  ...
  if (!existing) {
    return NextResponse.json({ error: "Active role was not found." }, { status: 404 });
  }

  const role = await prisma.userAppRole.update({
    where: { id: existing.id },
    data: { revokedAt: new Date(), assignedBy: allowed.user?.id ?? existing.assignedBy },
  });

  await logRoleRevoked({
